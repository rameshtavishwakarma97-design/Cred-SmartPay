...
import { getSmartRecommendation } from '../api.js';
...
export function renderRecommendation(app, navigate, params) {
  ...
  screen.innerHTML = `
    ...
    <div style="text-align: center; padding: 80px 24px;">
      ...
      <div style="font-family: var(--font-display); font-weight: 700; font-size: 1rem; margin-bottom: 8px;">Analyzing your cards...</div>
    ...
    <div class="screen-padding stagger-3" style="display: flex; flex-direction: column; gap: 10px;">
      ${reco.userCards.slice(1).map((r, i) => `
        <div class="reco-card" data-card-id="${r.card.id}">
          ...
          <div class="reco-breakdown" style="font-size: 0.7rem;">${r.label || r.reasoning || ''}</div>
          ${renderCapWarning(r)}
          <div class="savings-bar-container"><div class="savings-bar-bg"><div class="savings-bar-fill" style="width: 0%;" data-target="${maxBar > 0 ? Math.round((r.totalSavings / maxBar) * 100) : 0}"></div></div></div>
        </div>
      `).join('')}
    </div>
    ...
    ${reco.industryBest ? `
      ...
      <div class="screen-padding stagger-4" style="padding-bottom: 24px;">
        <div class="reco-card industry">
          ...
          <div style="font-family: var(--font-display); font-size: 1.5rem; font-weight: 800; color: var(--orange-sunshine); margin: 8px 0;">₹${reco.industryBest.savings} saved</div>
          ...
          ${reco.industryBeatUser ? `
            <div style="margin-top: 14px; padding: 10px 14px; background: rgba(255,107,44,0.06); border-radius: var(--radius-md); font-size: 0.72rem; color: var(--text-secondary); line-height: 1.6;">
              💡 <span style="color: var(--orange-sunshine); font-weight: 600;">Consider getting this card</span> — save <span style="color: var(--orange-sunshine); font-weight: 700;">₹${(reco.industryBest.savings - reco.bestUserCard.totalSavings).toFixed(0)} more</span> per transaction.
            </div>
          ` : `
